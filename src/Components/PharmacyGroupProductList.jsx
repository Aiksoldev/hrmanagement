...
const PharmacyGroupProductList = ({
  limit,
  listproductcount,
  ...
  const [initaialDate, setInitaialDate] = useState([
    {
      startDate: new Date(),
      endDate: new Date(),
      key: "selection",
  ...
  const handleReportAccordingToRange = () => {
    console.log(selectedDate);
    searchResult("", selectedDate.startDate, selectedDate.endDate);
    handleClose();
  };

  const handleDateRange = (item) => {
    setSelectedDate(item.selection);
    setInitaialDate([item.selection]);
  };
  ...
  return (
    ...
    >
      ...
      <DateRangePicker
        ...
        months={2}
        ranges={initaialDate[0].startDate === "" ? "today" : initaialDate}
      ...
      <div>
        <button
          style={{ float: "right" }}
          type="button"
          onClick={(e) => handleReportAccordingToRange(e)}
...
export default PharmacyGroupProductList;
